...
Devise.setup do |config|
  ...
  # will be used on authentication. The same considerations mentioned for
  ...
  config.pepper = Rails.application.secrets.devise[:pepper]
  ...
  omniauth = Rails.application.secrets.omniauth
  omniauth.each do |provider, configs|
    provider_configs = [provider.to_sym] + configs.values
    config.omniauth(*provider_configs)
  end
